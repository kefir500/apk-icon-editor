 ...
 */
bool JlCompress::compressFile(QuaZip* zip, QString fileName, QString fileDest, const int LEVEL) {
    ...
    if(!outFile.open(QIODevice::WriteOnly, QuaZipNewInfo(fileDest, inFile.fileName()), NULL, 0, METHOD, LEVEL)) return false;
 ...
 * * non si riesce a chiudere l'oggetto zip;
 */
bool JlCompress::compressDir(QString fileCompressed, QString dir, int level, bool recursive) {
    // Creo lo zip
    QuaZip zip(fileCompressed);
    QDir().mkpath(QFileInfo(fileCompressed).absolutePath());
    if(!zip.open(QuaZip::mdCreate)) {
    ...
    if (!compressSubDir(&zip,dir,dir,level,recursive)) {
        QFile::remove(fileCompressed);
        return false;
    }
